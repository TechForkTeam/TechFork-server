 ...
 */
class DeveloperTokenIntegrationTest extends MySqlRedisIntegrationTestBase {
    ...
    @Nested
    @DisplayName("POST /api/v1/admin/developer-token")
    class GenerateDeveloperToken {

        @Test
        @DisplayName("개발자 토큰 발급 성공 - ADMIN 권한으로 30일 만료 토큰 발급")
        void adminUser_ReturnsDeveloperToken() throws Exception {
            // When & Then
        ...
        void adminUser_ReturnsValidAccessToken() throws Exception {
        ...
        void normalUser_ReturnsForbidden() throws Exception {
            // When & Then
            mockMvc.perform(post("/api/v1/admin/developer-token")
        ...
        void noAuthentication_ReturnsUnauthorized() throws Exception {
            // When & Then
            mockMvc.perform(post("/api/v1/admin/developer-token"))
                    .andDo(print())
