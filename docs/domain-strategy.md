...
4. `PersonalizationProfileService`는 여전히 Activity/Post/Recommendation을 조합하는 애플리케이션 서비스 성격이 강하며, `PersonalizedProfileGenerated` 이벤트는 후속 분리 후보로 남아 있다.
5. `PersonalizationProfileDocument`는 독립 write aggregate보다 검색·추천용 read model에 가깝다.

향후 아래 조건이 충족되면 구현 경계를 더 세분화하는 것을 다시 검토한다.

1. Search/Recommendation이 개인화 프로필을 전용 포트/Published Language로 소비할 때
2. `PersonalizedProfileGenerated` 이벤트로 프로필 생성과 추천 생성을 분리할 때
3. 개인화 프로필이 독립 수명주기, 재생성 정책, 실패 복구 정책을 가진 모델로 커질 때
4. Auth cache 무효화가 outbox/retry 같은 별도 복구 정책을 필요로 할 때
...
- 장점
  ...
  - 개인화 프로필 생성처럼 비동기/외부 인프라 의존이 있는 후처리는 커밋 이후 실행된다.
  - 탈퇴 인증 캐시 무효화 실패는 커밋 전에 감지되어 탈퇴 트랜잭션을 롤백할 수 있다.
- 리스크
  - `Source ↔ Post`처럼 양방향 도메인 의존이 생긴다.
  - `PersonalizationProfileService`가 Activity, Post, Recommendation을 모두 알아 개인화 흐름의 결합도가 높다.
