...
- `PersonalizedProfileGeneratedEvent`로 프로필 생성과 추천 생성 트리거가 분리되었다.
...
2. 서버는 refresh token을 Redis에 저장하고 HttpOnly cookie로 내려준다.
...
| P0 | 개인화 프로필이 생성됨 | `PersonalizedProfileGeneratedEvent` | `PersonalizationProfileService.generatePersonalizationProfileSync` | 추천 생성, 개인화 검색 준비 완료 | 프로필 projection 저장 성공 후 발행된다. Recommendation 리스너가 `AFTER_COMMIT`에서 이벤트 스냅샷 기반 추천 생성을 수행한다. |
...
2. `PersonalizedProfileGeneratedEvent` — **도입 완료**
3. `TechnicalPostIndexed`

User Account 관련 이벤트는 Personalization/Auth 후처리와 분리되었고, Personalization Profile 생성 완료도 Recommendation 후처리와 분리되었다. 남은 핵심 이벤트는 **검색/추천 콘텐츠 준비 완료 명시화**에 집중한다.
...
- Activity 바운디드 컨텍스트 내부에서는 다른 컨텍스트 조회를 `UserLookupService`, `PostLookupService`, `PostKeywordLookupService`, `BookmarkLookupService` 같은 application 서비스로 통일한다.
- hexagonal architecture(포트/어댑터) 적용은 이 DDD 정리와 application 경계 정리가 끝난 뒤 후속 단계에서 검토한다.
...
// 발행 지점 예시 (서비스 레이어)
applicationEventPublisher.publishEvent(new PersonalizedProfileGeneratedEvent(userId, profileVector, keyKeywords));
...
`PostDocument`, `PersonalizationProfileDocument`는 검색/추천을 위한 read model projection이다. 특히 `PersonalizationProfileDocument`는 Personalization Profile 컨텍스트가 생성하지만 Aggregate가 아니며, Search/Recommendation은 이를 읽는 소비자일 뿐 소유하지 않는다.
