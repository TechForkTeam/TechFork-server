...
4. 다음 분리 후보는 `PersonalizationProfileService -> RecommendationService` 직접 호출을 `PersonalizedProfileGenerated` 이벤트/포트로 전환하는 것이다.
```
...
- 현재는 `domain/personalization` 패키지로 분리되어 있고, User Account 서비스의 직접 호출 결합은 이벤트 리스너로 분리되었다.
...
- 후속 작업은 `PersonalizedProfileGenerated` 이벤트로 개인화 프로필 생성과 추천 생성을 분리하는 것이다.
...
[후속] PersonalizedProfileGenerated
...
- User Account 이벤트는 완료. 남은 `PersonalizedProfileGenerated`, `TechnicalPostIndexed`는 Phase 6 진입 조건 체크리스트를 계속 확인

목표:
- 완료된 User Account 이벤트 seam을 기준으로, 남은 이벤트를 순서대로 도입해 컨텍스트 결합도를 낮춘다.
